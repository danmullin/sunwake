...
/** @type {"nightDrive" | "rainDrive" | "skyline" | "tunnel"} */
let vizMode = "nightDrive";
const VIZ_MODES = ["nightDrive", "rainDrive", "tunnel", "skyline"];
try {
  const stored = localStorage.getItem(VIZ_MODE_KEY);
  if (["skyline", "nightDrive", "rainDrive", "tunnel"].includes(stored)) vizMode = stored;
} catch {
  /* ignore */
}
...
function syncVizModeUi() {
  ...
  if (vizSwitchBtn) {
    ...
    const labels = { nightDrive: "Night Drive", rainDrive: "Rain Drive", tunnel: "Tunnel", skyline: "Skyline" };
    vizSwitchBtn.textContent = labels[nextMode] ?? "Night Drive";
    vizSwitchBtn.title = `Switch to ${labels[nextMode] ?? "Night Drive"}`;
  }
  // Tuck Effects panel in non–Night Drive modes (Skyline, Tunnel)
  if (sky || tun) {
