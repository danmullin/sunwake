/**
 * Storm helpers — leaf module: state/math/camera only, no scene imports.
 * Imported by nightDrive.js, rainDrive.js, and simulation hook registration.
 */
import {
  ctx, W, H, FX, vizMode,
  stormClouds, lightningBolts, LIGHTNING_MAX, rainSplashes, RAIN_SPLASH_MAX,
  stormFlash, setStormFlash, setLastLightningAt,
} from "./state.js";
import { vanishX } from "./camera.js";

export function seedStormClouds() {
...
export function jaggedBoltPath(x0, y0, x1, y1, segs, jag) {
  const pts = [{ x: x0, y: y0 }];
...
export function spawnLightning(strength, kind = "kick") {
  ...
  setStormFlash(Math.min(1, stormFlash + 0.45 + strength * 0.5));
}

export function spawnRainSplash(strength) {
...
export function drawStormSky(now, bass, mid) {
...
export function drawStormClouds(now, bass, mid) {
...
export function strokeBolt(pts, width, color) {
...
/**
 * Age + prune storm clouds, lightning bolts, and rain splashes. Only Rain
 * Drive drifts clouds and cracks lightning; the flash still decays and the
 * splash/bolt pools still drain on the way out of the mode.
 */
export function tickStorm() {
...
export function drawWetAsphalt(now, bass, mid, air) {
  const horizon = H * 0.52;
  ...
  ctx.globalCompositeOperation = "lighter";
  const vx = typeof vanishX === "function" ? vanishX() : W * 0.5;
  for (let i = 0; i < 7; i++) {
    const t = i / 6;
    const x = vx + (t - 0.5) * W * (0.55 + bass * 0.15);
