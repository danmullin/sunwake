...
const VIZ_MODES = ["nightDrive", "rainDrive", "tunnel", "arcade", "skyline"];
try {
  const stored = localStorage.getItem(VIZ_MODE_KEY);
  if (["skyline", "nightDrive", "rainDrive", "tunnel", "arcade"].includes(stored)) vizMode = stored;
} catch {
  /* ignore */
}
...
/** Tunnel mode state — scrolling rib lights racing toward camera. */
let tunnelScroll = 0;
let tunnelPulse = 0;          // bass pulse ring amplitude (0..1)
let tunnelSway = 0;           // lateral drift with camera
const tunnelPulseRings = [];  // [ { r, life, decay, hue, strength } ]
const TUNNEL_RINGS_MAX = 24;

/** Arcade Cabinet — CRT screen + vector starfield + coin-op EQ chrome. */
const ARCADE_EQ_N = 32;
const arcadeEq = new Float32Array(ARCADE_EQ_N);
let arcadeWarp = 0;
let arcadeFlash = 0;
const arcadeStars = [];
const ARCADE_STAR_N = 160;
...
const TUNNEL_RIBS = 28;       // ribs visible at once (some off-screen back)
const TUNNEL_RIB_SPACING = 0.072; // world-space depth between ribs (0..1)
const TUNNEL_FOV = 0.72;      // perspective divisor feel

/** Convert normalised depth (0=camera, 1=far) → screen scale + position. */
...
/**
 * Arcade Cabinet — CRT bezel, scanlines, vector starfield, EQ as coin-op chrome.
 */
...
/** Arcade sim — EQ bars, star seed, warp / flash (kept out of draw). */
function updateArcadeCabinet(now, bass, mid, _air, peak, snare, _hat, solo) {
  ...
  arcadeWarp = smooth(arcadeWarp, drive, 0.22);
  arcadeFlash = smooth(arcadeFlash, peak > 0.45 || snare > 0.55 ? 0.55 + peak * 0.45 : 0, 0.35);
}
...
/** Tunnel sim — scroll, sway, bass pulse rings (kept out of draw). */
function updateTunnel(_dt, bass, mid, air, peak, _snare, _hat, _solo) {
  const driveTarget = 0.008 + bass * 0.05 + mid * 0.018 + peak * 0.025;
  tunnelScroll = (tunnelScroll + smooth(0, driveTarget, 0.28) + driveTarget) % 1;
  tunnelSway = smooth(tunnelSway, (bass - mid) * 0.08, 0.04);
  tunnelPulse = smooth(tunnelPulse, bass * 0.9 + peak * 0.35, 0.18);
...
/**
 * The tunnel — perspective octagonal ribs racing toward camera, neon rail
 * lines, bass-driven pulse rings, and a dark void at the far end.
 */
