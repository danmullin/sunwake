...
function updateArcadeCabinet(now, bass, mid, _air, peak, snare, _hat, solo) {
  ...
  setArcadeWarp(smooth(arcadeWarp, drive, 0.22));
  setArcadeFlash(smooth(arcadeFlash, (peak > 0.45 || snare > 0.55) ? 0.55 + peak * 0.45 : 0, 0.35));
}
...
/** Coin-op arcade cabinet with vector starfield and CRT sun. */
export class ArcadeScene extends VizScene {
  update(now, dt, levels, fx) {
