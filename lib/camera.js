...
export function updateCamera(now, bass, mid, air, peak, snare) {
  ...
  if (fxOn("cameraSway")) {
    ...
    // Zoom must cover translate + bank — 1.04 alone leaves side slabs that
    // never get redrawn; with alpha:false those slabs keep prior frames and
    // additive ("lighter") bloom washes the whole canvas white.
    const span = Math.min(W || 1, H || 1);
    const cover =
      1 +
      (Math.hypot(tx, ty) * 2.35) / span +
      Math.abs(trot + tbank) * 1.55;
    const pulse = 1.04 + (bass * 0.032 + peak * 0.018 + air * 0.008) * d;
    tzoom = Math.max(cover, pulse);
  }
