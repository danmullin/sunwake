/**
 * Grid / sea domain — the perspective floor Night Drive drives over: lit
 * flocks + constellation trails, vanishing meteors, mirror sea reflection,
 * bass mountain skyline, grid heartbeat ECG, horizon bloom rings, and the
 * drawSea/drawHorizon renderer that composites all of them onto the mesh.
 * One cohesive system — kept as one file on purpose, the way it's one
 * system visually.
 */
import {
  ctx, W, H, vizMode, playing, fxOn, FX,
  SUN_SCALE, WHIP_VERTICALS, WHIP_CREST_WIDTH, WHIP_STACK,
  GRID_ROWS, GRID_COLS, GRID_CELL_MAX, GRID_TRAIL_MAX, METEOR_MAX,
  MIRROR_MAX, HEARTBEAT_MAX, BLOOM_MAX, GRID_SUN_COL, BASS_MOUNTAIN_N,
  GRID_CELL_STEP_MS, GRID_RAINBOW,
  ribbons, gridCells, gridTrails, meteors, mirrorCells, heartbeats, bloomRings, bassMountain,
} from "../state.js";
import { PERF } from "../perf.js";
import { synthRainbow, ecgShape } from "../math.js";
import { vanishX } from "../camera.js";
import { sunAnchor } from "./sun.js";

// ─── Lit flocks / constellation trails ───────────────────────────────────────

export function spawnGridFlock(size, { row, col, hue, strength } = {}) {
...
export function stepGridCell(cell) {
...
export function spawnGridCells(n, opts = {}) {
...
export function spawnVanishingMeteor(opts = {}) {
...
export function spawnVanishingMeteors(n, opts = {}) {
...
export function spawnGridHeartbeat(strength = 0.6) {
...
export function spawnHorizonBloom(strength = 0.6) {
...
export function spawnMirrorSea() {
...
export function mirrorMeshPoint(depth01, colI) {
...
// ─── Bass mountain skyline ────────────────────────────────────────────────────

export function bassMountainProfile(i, now = 0) {
...
export function updateBassMountain(bass, kickBoost, now) {
...
export function drawBassMountain(bass) {
...
// ─── Music energy predicates (used by the drum router) ──────────────────────

export function gridMusicEnergy(bass, mid, air, snare = 0) {
  return bass * 0.42 + mid * 0.28 + air * 0.12 + snare * 0.38;
}

export function gridMusicHot(bass, mid, air, snare = 0) {
  return playing && gridMusicEnergy(bass, mid, air, snare) > 0.1;
}

// ─── Horizon glow + the sea grid renderer ────────────────────────────────────

export function drawHorizon(bass) {
  const y = H * 0.52;
...
export function drawSea(now, bass, mid, air) {
  const horizon = H * 0.52;
  const seaH = H - horizon;
...
/** Age + prune every pool that rides the sea mesh. Takes the frame's dt (ms). */
export function tickGridSea(dt) {
...
/** Hard-clear every mesh pool — used when playback stops. */
export function clearGridSea() {
  gridCells.length = 0;
  gridTrails.length = 0;
  meteors.length = 0;
  mirrorCells.length = 0;
