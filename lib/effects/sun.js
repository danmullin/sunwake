/**
 * Sun / black-hole domain — geometry, disk rendering, petals, quasar jets,
 * flares, halo rings, and the accretion "infall" sparks that only run while
 * Black Hole is armed. Everything here shares one anchor point and one set
 * of size constants, so it stays together instead of being split by
 * "particle vs. not" the way the old particles.js / sun.js split it.
 */
import {
  W, H, ctx, FX, CAM, sourceMode,
  SUN_SCALE, SUN_Y_FRAC, SUN_DROP_PER_EXTRA,
  BH_DISK_TILT, fxOn, infalls, INFALL_MAX,
} from "../state.js";
...
export function drawSoftSun(bass, mid, solo = 0) {
  const { x, y } = sunAnchor();
  ...
  if (fxOn("blackHole")) {
    ...
    const pulseGlow = fxOn("photonPulse") ? FX.photon : 0;
    ctx.save();
    ctx.translate(x, y);
    ctx.globalCompositeOperation = "lighter";
    const ringGlow = 0.4 + ss * 0.25 + pulseGlow * 0.55;
    ctx.strokeStyle = `rgba(255, 230, 200, ${Math.min(0.95, ringGlow)})`;
    ctx.lineWidth = Math.max(1.5, r * (0.035 + pulseGlow * 0.05));
    ctx.beginPath();
    ctx.arc(0, 0, voidR * (1.03 + pulseGlow * 0.02), 0, Math.PI * 2);
    ctx.stroke();
    ctx.strokeStyle = `rgba(255, 140, 100, ${0.28 + mm * 0.2 + pulseGlow * 0.4})`;
    ctx.lineWidth = Math.max(2, r * (0.06 + pulseGlow * 0.08));
    ctx.beginPath();
    ctx.arc(0, 0, voidR * (1.08 + pulseGlow * 0.025), 0, Math.PI * 2);
    ctx.stroke();
    if (pulseGlow > 0.35) {
      ctx.strokeStyle = `rgba(255, 255, 255, ${(pulseGlow - 0.35) * 0.9})`;
...
export function drawSunPetals(now, mid, solo) {
...
export function drawQuasarJets(now, bass, mid, solo) {
...
export function drawSunFlares(now, peak, solo, bass) {
...
export function drawHeartbeatRing(bass, mid) {
...
export function drawUsPresence(bass, mid, air) {
...
// ─── Black-hole accretion sparks ─────────────────────────────────────────────
// Only fire while Black Hole + Infall Sparks are both armed (see sunAnchor()
// use for spawn geometry above).

export function spawnInfall(strength = 0.5) {
...
export function drawInfallSparks(bass = 0, solo = 0) {
...
/** Age + prune infall sparks. Called once per frame regardless of play state. */
export function tickInfallSparks() {
